...
1. Download the [Data import cheat sheet](https://rstudio.github.io/cheatsheets/html/data-import.html)
1. Create a new quarto file called `app-import.qmd`
1. Update the YAML header 
1. Replace the setup chunk with the one below

You should have all of these packages installed already, but if you get the message `Error in library(x) : there is no package called ‘x’`, please refer to @sec-install-package.
...
### New Script {#sec-exercises-new-qmd-4}

Create and save a new quarto document named `starwars_report.qmd`. In the set-up code chunk load the packages `tidyverse` and `rio`.
...
* Hide the code and any messages from the rendered output
