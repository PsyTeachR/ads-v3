...
Saving or displaying the animation can take 10-20 seconds or more to render, so it's usually best to create the plot interactively, save it to a gif with `anim_save()`, and set the relative code chunk to `#| eval: false` so you can load the gif as an image when you render, rather than render it each time.
...
```{r}
#| label: gganimate-demo
#| fig-cap: "Animated plot demo using gganimate."
