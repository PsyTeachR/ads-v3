...
* tidyverse (you can use all these with `library(tidyverse)`)
    ...
    * dplyr:: `dplyr::count()` 
    * ggplot2:: `aes()`, `coord_cartesian()`, `element_blank()`, `facet_wrap()`, `geom_bar()`, `geom_boxplot()`, `geom_col()`, `geom_histogram()`, `geom_jitter()`, `geom_point()`, `geom_smooth()`, `ggplot()`, `ggsave()`, `ggtitle()`, `guides()`, `scale_fill_manual()`, `scale_x_continuous()`, `scale_x_discrete()`, `scale_y_continuous()`, `spec()`, `theme()`, `theme_bw()`, `theme_minimal()`, `theme_set()`
...
1. Download the [ggplot2 cheat sheet](https://rstudio.github.io/cheatsheets/html/data-visualization.html)
...
Let's build up the plot above, layer by layer. First we need to get the data. You can see how to load data from different sources in @sec-import, but this time we'll use the same method as we did in @sec-loading-online and load it from an online source. 
...
In a new code chunk, create a blank plot.
...
Part of the power of <pkg>ggplot2</pkg> is that you can add more than one geom to a plot by adding on extra layers and so it quickly becomes possible to make complex and informative visualisations. Importantly, the layers display in the order you set them up. The code below uses the same geoms to produce a boxplot with the raw data points, but orders them differently.
...
#| fig-cap: "Points on top vs points underneath"
#| fig-alt: "Two nearly identical plots with boxplots plus raw data points. In the left plot, the points are underneath the boxplot, and in the right plot they are on top."
#| message: false
#| echo: false 
...
points_top + points_bottom + plot_layout(nrow = 1)
```
...
```{r}
#| eval: false
points_top # view first plot
points_bottom # view second plot
```
...
#| fig-alt: "The points_top and points_bottom plots displayed in one image side-by-side."
#| message: false
# add plots together in 1 row
points_top + points_bottom + plot_layout(nrow = 1)
```
...
1. Combine your plots using `plot_layout()`. 
2. Try changing the value of `nrow` to 2.
3. Add the argument `axes = "collect"` to `plot_layout()`. Use the help documentation for additional explanation on what it does.
:::

The [`patchwork` documentation](https://patchwork.data-imaginist.com/articles/guides/layout.html) has some great resources and examples of all the different ways in which you can adjust the layout when you combine multiple plots.
...
1. Update the ylim limits of `mean_welfare` as above.
2. Then, update the xlim of `gini` according to their minimum and maximum values (these were described earlier). This plot looks quite strange and we wouldn't recommend this is how you present the data, but consider the story it tells and how it might change your interpretation.
...
1. Save two copies of a plot you have made from this chapter with different sizes.
2. Make sure you can find them on your computer - they should be stored in your project folder.
...
```{r}
ggplot(count_data, aes(x = fct_reorder(region_code, # variable to reorder
                                       n), # what to order it by
                       y = n)) +
  geom_col() +
  labs(x = "Region")
```
...
In @fig-layers, we emphasised the relationship between Gini index and welfare with a trendline created by `geom_smooth()`.
...
## Explain what the data simulation is doing

1. This fixes the random-number generator so that every time you run the simulation, you get the same random values. This is essential for reproducibility, especially in teaching materials.
...
2. This sets the number of simulated data points you want to create.
You will end up with 10,000 rows in your final tibble.
...
3. This creates a huge sequence of datetimes:
...
4. The simulated dataset is created using `tibble()`, which lets you define each column directly. The first column, `datetime`, is generated by taking 10,000 random samples from the long vector of `datetimes` using `sample()`, so each observation receives a timestamp somewhere in 2025. 
...
Change the values in this code (range between 0 and 1) to understand what jitter is doing
...
- Advanced: Styling Plots ([@sec-plotstyle])
- Advanced: More Plots ([@sec-more-plots])
...
2. Create and save a new quarto document named `03-dataviz-exercises.qmd`. Give it the title "Personality Data" and remove the default template text.

3. Add the code below in the set-up code chunk:
...
```
````
4. Give human-readable labels to the columns you'll be plotting:
...
6. Create a visualisation with an appropriate plot style for each question below:

* What does the distribution of scores look like for Agreeableness (`Ag`)?
* Are Extraversion (`Ex`) and Openness (`Op`) correlated?

7. Duplicate the code for your first plot and create the same for each of the five personality factors. Combine these 5 plots into a single figure.

8. Write a short summary that interprets the data shown in the plots - it's not enough just to present visualisations, effective reports will also help the reader understand the conclusions they should draw from the plots you've presented. Organise your report so that the plots are shown **after** the text summary.

9. Edit the YAML header of your quarto document so that your rendered report does not show any code, messages, or warnings.

10. Pick one plot above and make a new version. Adjust the visual aesthetics to make it look as bad and as difficult to read as possible. Save the plot as a PNG image then post this plot to the week 3 thread on Teams and explain why it is so bad. Include the code in your post. 
