...
This appendix will introduce you to some more plot styles, like maps and word clouds, and things you can do with plots, such as add annotations.
...
1. Create a new quarto file called `adv-plots.qmd`
...
#| fig-width: 8
#| fig-height: 3
#| fig-cap: "Bump plot with added customisation"
ggplot(data = rank_data, 
       mapping = aes(x = week, 
                     y = rank, 
                     colour = person,
                     label = person)) + # <1>
  ggbump::geom_bump(linewidth = 10) + # <2>
  scale_x_continuous(name = "",
                     breaks = 1:3, # <3>
                     labels = c("Week 1", "Week 2", "Week 3"),
                     expand = expansion(c(.05, .2))) +
  scale_y_reverse(name = "Ranking", # <4>
                  breaks = 1:3, 
                  expand = expansion(.2)) + # <5>
  geom_text(data = filter(rank_data, week == 3), # <6>
            color = "black", 
            x = 3.05, # <7>
            hjust = 0) + # <8>
  theme(legend.position = "none", # <9>
        ...
        axis.text.x = element_text(size = 12)) # <10>
```

1. Add `label = person` to the mapping so we can add in text labels.
2. Increase the size of the lines with the `linewidth` argument to `geom_bump()`
3. We don't need labels for weeks 1.5 and 2.5, so change the x-axis `breaks`
4. It makes more sense to have first place at the top, so reverse the order of the y-axis with `scale_y_reverse()` and fix the breaks and expansion.
5. The `expand` argument for the two scale_ functions expands the plot area so we can fit text labels to the right.
6. Add text labels with `geom_text()`, but just for week 3, so set `data =  filter(rank_data, week == 3)` for this geom. 
7. Set `x = 3.05` to move the text labels just to the right of week 3
8. Set `hjust = 0` to right-justify the text labels (the default is `hjust = 0.5`, which would center them on 3.05).
9. Remove the legend and grid lines. 
10. Increase the x-axis text size.


#### Word Clouds

Word clouds are a common way to summarise text data. First, download <a href="https://psyteachr.github.io/ads-v3/data/amazon_alexa.csv" download>amazon_alexa.csv</a> into your data folder and then load it into an object. This dataset contains  text reviews as well as the 1-5 rating from customers who bought an Alexa device on Amazon.
...
```{r}
words <- alexa |>
  tidytext::unnest_tokens(output = "word", input = "verified_reviews")
```
...
```{r}
rating1 <- filter(words, rating == 1) |> # <1>
  ggplot(aes(label = word,               # <2>
             colour = "red",             # <3>
             size = n)) +                # <4>
  geom_text_wordcloud_area() +           # <5>
  scale_size_area(max_size = 10) +       # <6>
  ggtitle("Rating = 1") +
  theme_minimal(base_size = 14)
```

1. Filter retains only the data for 1-star ratings. 
2. `label` comes from the `word` column and is the data to plot (i.e., the words).
3. `colour` makes the words red (you could also set this to `word` to give each word a different colour or `n` to vary colour continuously by frequency).
4. `size` makes the size of the word proportional to `n`, the number of times the word appeared.
5. `geom_text_wordcloud_area()` is the word cloud geom. 
6. `scale_size_area()` controls how big the word cloud is (this usually takes some trial-and-error).  

```{r}
#| label: fig-word-cloud
#| fig-cap: "A word cloud of 1-star ratings"
rating1
```
...
```{r}
#| label: fig-map-islands
#| fig-width: 6
#| fig-height: 6
#| fig-cap: "Map coloured by country."
...
You can join <a href="https://psyteachr.github.io/ads-v3/data/scottish_population.csv" download>Scottish population data</a> to the map table to visualise data on the map using colours or labels.
...
#| fig-width: 5
#| fig-height: 7
#| fig-cap: "Map coloured by population."
# plot
ggplot() + 
  geom_sf(data = scotmap_pop,
          mapping = aes(fill = population), # <1>
          color = "white", 
          size = .1) +
  coord_sf(xlim = c(-8, 0), ylim = c(54, 61)) +
  scale_fill_viridis_c(name = "Population", # <2>
                       breaks = seq(from = 0, to = 6e5, by = 1e5),  # <3>
                       limits = c(0, 6e5), # <4>
                       labels = paste0(0:6, "00 K")) + # <5>
  theme(legend.justification = c(0,1), # <6>
        legend.position = c(0, 1)) 
```

1. Setting the fill to population in `geom_sf()` gives each region a colour based on its population. 
2. The colours are customised with `scale_fill_viridis_c()`. 
3. The breaks of the fill scale are set to increments of 100K (1e5 in scientific notation) 
4. The scale is set to span 0 to 600K. 
5. Creates the labels by taking the numbers 0 through 6 and adding "00 k" to them.
6. The position of the legend is moved into the sea by setting `legend.justification` and `legend.position` to c(0,1), which places the top left corner of the legend at the top left corner of the plot..
...
* Use `anim_save()` to save the animation to a GIF file and set this code chunk to `#| eval: false` because creating an animation takes a long time and you don't want to have to run it every time you render your report.
