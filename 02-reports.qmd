...
Before we write any code, first, we need to get organised. `r glossary("project", "Projects")` in RStudio are a way to group all the files you need for one project. Most projects include `r glossary("script", "scripts")`, data files, and output files like images or the PDF report created by the script.
...
* Name the project `r path("ADS-2026")`
* Save it inside the `R-projects` directory
...
![](images/reports/new_proj_2.png){fig-alt="Next page of the New Project Wizard interface. There is a back button and the heading Project Type. There are 7 options: New Project, R Package, Shiny Application, Quarto Project, Quarto Website, Quarto Blog, and Quarto Book. The first option is highlighted with the help text `Create a new project in an empty directory.'"}
...
Click on the Files tab in the lower right pane to see the contents of the project directory. You will see a file called `ADS-2026.Rproj`, which is a file that contains all of the project information. When you're in the Finder/Explorer, you can double-click on it to open up the project.
...
-   use underscores (`_`) to separate parts of the file name, like the title and date, and dashes (`-`) to separate words in each part (e.g., `thesis-analysis_2024-10-31.qmd`)
...
This is the `r glossary("YAML")` header, which provides information to quarto about how you want to render a document. Here, it sets the title, author, and format. 

::: {.callout-tip .activity}


Add a new line with the date, e.g., `date: 2026-01-20`.
...
1. First, you can highlight the code you want to run and then click <if>Run > Run Selected Line(s)</if>, however this is tedious and can cause problems if you don't highlight *exactly* the code you want to run.
...
3. Click the green arrow to run all the code in the current chunk.
...
1. Run your code using each of the methods above. You should see the variables `name`, `age`, `today`, and `halloween` appear in the environment pane. 

2. Restart R to clear the objects. They should disappear from the environment (see @sec-rstudio-settings if they don't disappear). 

3. Run your code again, and then change the value of `name` in the script. When/how does it change in the Environment tab? 
:::
...
It is `r halloween - today` days until Halloween, which is my favourite holiday.
```
...
Now we are going to `r glossary("render")` the file into a document type of our choosing. In this case we'll create a default html file, but you will learn how to create other files like Word and PDF in @sec-formats. 

::: {.callout-tip .activity}


To render your file, click the <if>Render</if> button at the top of the source pane.
...
> Error in read_csv("https://psyteachr.github.io/ads-v3/data/smalldata.csv") :  
...
In your `r path("ADS-2026")` directory, create a new folder named `r path("data")`, [download a copy of the data file](https://psyteachr.github.io/ads-v3/data/smalldata.csv){download=""}, and save it in this new subdirectory. Or you can run the following code in the console (do not add it to the script or you will unnecessarily download data every time you run it).

```{r}
#| eval: false
#| code-summary: "Run in the console"
dir.create(path = "data", showWarnings = FALSE)
url <- "https://psyteachr.github.io/ads-v3/data/smalldata.csv"
...
1. Create a code chunk to display a graph of the data in your document after the text we've written so far. We'll use some code that you'll learn more about in @sec-viz to make a simple bar chart that represents the data -- focus on trying to follow how bits of the code map on to the plot that is created.

2. Add a new level-3 header called "Visualisation". Copy and paste the code below into a new chunk. Run the code in your script to see the plot it creates and then render the file to see how it is displayed in your document.

```{r}
#| label: smalldata-fig
#| fig-alt: "A plot with pre-test score on the x-axis and post-test score on the y-axis, both running about 70 to 130. The control group is represented by red ddots, the experiemntal group by blue. There are 5 dots of each, with what looks like a positive relationship."
...
``` md
![ADS](https://psyteachr.github.io/images/ads.png){width="200px" fig-alt="A rainbow-bordered hexagon with the words 'Applied Data Skills"}
```
...
```{r}
#| echo: false
#| fig-alt: "Screenshot of Google Images interface with Usage Rights selections open."
...
1. Add a new level 3 heading to your document, name the heading "Tables" and then create a new code chunk below this. 

2. Then, let's see what the table looks like if we don't make any edits. Simply write the name of the table you want to display in the code chunk (in our case `smalldata`) and then render to see what it looks like.
...
#| fig-alt: The same plot as above, but the text and points are larger in proportion to the full plot size
...
#| fig-alt: The same plot as above, with the same internal protoprtions as the original plot, but displayed smaller at half the page width
ggplot2::last_plot()
```
...
For example, by default you might want to hide your code but there also might be an occasion where you want to show the code you used to analyse your data. You can set `#| echo: false` in your setup chunk to make hiding code the default but in the individual code chunk for your plot set `#| echo: true`. 

::: {.callout-tip .activity}


Try this now and render the file to see the results.
:::
...
#| fig-cap: Relationship between pre- and post-test by group
#| fig-width: 10
#| fig-height: 5
ggplot(data = smalldata, 
       mapping = aes(x = pre, 
...
Finally, the `r glossary("YAML")` header is the bit at the very top of your quarto document. You can set several options here as well. See the [quarto HTML reference](https://quarto.org/docs/reference/formats/html.html) for more options.
...
Try changing the theme to see which one you like best.
...
### Embedding Resources {#sec-embed-resources}

When you create an HTML file, there are helper files that control the look (css) and functionality (js) of the web page, as well as image files. When you render an HTML file in quarto, these helper files are saved in a folder named `{report_name}_files`. It is efficient to have a single copy of these that are linked to all your pages in a big website, but this means you always have to share this folder alongside your html file, and if they are not in the same directory, the images and styles in your report will be broken.

To save all of the resources inside the html file, so you can ust share a single file, set `embed-resources: true` in the YAMl header.
...
::: {.callout-tip .activity}
Check the file size of the `02-reports.html` file in your project. Move it to another folder, like your Desktop, and see what it loos like when you open it in a web browser. 

Delete the `02-reports_files` directory, change the YAML header to embed resources, and re-render the script. 

Now check the size of the `02-reports.html` file. What happens if you move this file to another folder and open it?
:::
...
So far we've just rendered to html. To generate PDF reports, you need to install <pkg>tinytex</pkg> [@R-tinytex].

::: {.callout-tip .activity}


Run the following code in the console (do **not** add this to your qmd file):
...
Once you've done this, update your YAML heading to add a `pdf_document` section and render a PDF document. The options for PDFs are more limited than for HTML documents, so if you just replace `html` with `pdf`, you may need to remove some options if you get an error that looks like "Functions that produce HTML output found in document targeting PDF output."
...
- Advanced: Referencing ([@sec-bibliography])
- Advanced: Table formatting ([@sec-tables])

## Peer Coding Exercises {#sec-exercises-reports}
...
The goal this week is for each person to create and render their own reproducible CV in Quarto while collaborating to troubleshoot, check reproducibility, and give feedback.

You're still going to assume the roles of Driver and Navigator but you'll both do each step. The Driver will do it first, with the Navigator providing feedback as usual, and then the Navigator will quickly reproduce the same step having learned from the process of the Driver.
...
2. Create a new project called "cv".
3. In the "cv" project, create a new quarto document called "cv.qmd". Edit the YAML header to print data frames using kable and set a custom theme.
4. Write a short paragraph describing you and your work or academic aspirations. Include a bullet-point list of links to related websites, for example:

```
I am a research psychologist who is interested in open science and teaching computational skills.

* [psyTeachR books](https://psyteachr.github.io/)
* [Google Scholar](https://scholar.google.com/)
```

5. Make a subheading titled "Education" and use the following code to load a small table of your education. Edit it to be relevant to you (you can change the categories entirely if you want, for example, you could list your courses you have taken at University).  
...
6. Change the settings so that code chunks don't show in your rendered document.

7. Add an image of anything relevant. Make sure to add alt-text.

8. Use inline R to include the current date (pulled from the computer system) in a sentence like:

> This CV was created on `r Sys.Date()`.

9. Render this document to html ([@sec-render]).

10. Once you're done, zip up your entire project folder and share it on Teams under the week 2 exercise post. Try downloading someone else's project and rendering their qmd file.
